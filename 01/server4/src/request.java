...
public class request {
    ...
    private void getmethod(String info) {
        ...
        int tag = info.indexOf("?");
         this.URL = info.substring(begin + 1, end).trim();
        ...
        if (method.equals("POST")) {
            ...
            if (-1 != tag) {
                ...
                URL=url1;
                String par1 = info.substring(tag + 1, end).trim();
            ...
            if (-1 != tag) {
                String url1 = info.substring(begin + 1, tag).trim();
                URL=url1;
                System.out.println("url->" + URL);

                System.out.println(tag+1+"----"+end);
                 par = info.substring(tag + 1, end).trim();
